...
void MeterWidget::analogMeter(uint16_t x, uint16_t y, float startScale, float endScale, const char *units, const char *s0, const char *s1, const char *s2, const char *s3, const char *s4)
{
  ...
  updateNeedle(0, 0, 0, "%d");
}
    
bool MeterWidget::is_d_format(const char *fmt) {
    size_t len;
    return  fmt                     &&
            fmt[0] == '%'           &&
            (len = strlen(fmt)) > 1 &&
...
// #########################################################################
void MeterWidget::updateNeedle(float val, float showVal, uint32_t ms_delay, const char *format)
{
  int value = (val - scaleStart) * factor;

  ntft->setTextColor(TFT_BLACK, TFT_WHITE);
  char buf[12]; //dtostrf(showVal, 5, floatSize, buf);
  
  if(is_d_format(format)) {
    snprintf(buf, sizeof(buf), format, (int)showVal);
  } else {
    snprintf(buf, sizeof(buf), format, showVal);
  }
